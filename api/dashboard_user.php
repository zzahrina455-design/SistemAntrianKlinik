...
<body>
        ...
        <div class="container">
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-2 rounded-3 z-3">
                    ...
                    <li><a class="dropdown-item text-danger fw-bold" href="/api/logout.php"><i class="bi bi-box-arrow-right me-2"></i>Keluar</a></li>
                </ul>
            </div>
        </div>
    </nav>
        ...
        <div class="row justify-content-center g-4">
            ...
            <div class="col-md-7">
                <div class="content-card">
                    ...
                    <?php else: ?>
                        <form method="POST" action="/api/proses_antrian.php">
                            <div class="row">
    ...
    <script>
        function loadBpsData() {
            ...
            container.innerHTML = `<div class="py-4"><div class="spinner-border text-info" role="status"></div><p class="mt-2 text-muted fw-bold">Menarik data dari server BPS RI...</p></div>`;
            fetch('/api/api.php')
                .then(r => r.json())
                .then(data => {
                    if (btnRefresh) btnRefresh.innerHTML = '<i class="bi bi-arrow-clockwise"></i> Refresh';
                    if (data['data-availability'] === 'available') {
                        const key = Object.keys(data.datacontent)[0];
                        const ahh = data.datacontent[key];
                        container.innerHTML = `<div class="py-3"><h5 class="text-muted fw-normal mb-2">Angka Harapan Hidup (AHH) Nasional</h5><h1 class="display-3 fw-bold text-info mb-3">${ahh} Tahun</h1></div>`;
                    } else {
                        container.innerHTML = '<div class="text-muted py-4">Data BPS tidak tersedia.</div>';
                    }
                })
                .catch(() => {
                    if (btnRefresh) btnRefresh.innerHTML = '<i class="bi bi-arrow-clockwise"></i> Refresh';
